...
function TotalStock() {
  ...
  const [filters, setFilters] = useState({
    ...
    limit: 10, // Limit for stocks
    page: 1, // Page for stocks
  });

  const [stockOutFilters, setStockOutFilters] = useState({
    keyword: "",
    date: "",
    limit: 10, // Limit for stock-outs
    paymentStatus: "all", // paid | unpaid | all
    page: 1, // Page for stock-outs
  });
  ...
  const [stockOutTotalPages, setStockOutTotalPages] = useState(0);
  const [stockOutTotalRecords, setStockOutTotalRecords] = useState(0);
  ...
  const fetchStocks = async () => {
    ...
    try {
      ...
      setStockTotalPages(res.data.meta.totalPages);
      setStockTotalRecords(res.data.meta.totalRecords);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const fetchStockOut = async () => {
    ...
    try {
      ...
      setStockOutTotalPages(res.data.meta.totalPages);
      setStockOutTotalRecords(res.data.meta.totalRecords);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  // ================= USE EFFECT =================
  useEffect(() => {
    fetchStocks(); // Fetch stocks when stockFilters change
    fetchStockOut(); // Fetch stock-out data when stockOutFilters change
    fetchStockSummary(); // Fetch stock summary on initial load
  }, [filters, stockOutFilters]); // Dependency on both stockFilters and stockOutFilters
  ...
  const handlePageChange = (type, direction) => {
    ...
    if (type === "stockOut") {
      setStockOutFilters((prevFilters) => {
        const newPage =
          direction === "next" ? prevFilters.page + 1 : prevFilters.page - 1;
        return { ...prevFilters, page: newPage };
      });
  ...
  const handleDeleteStockOut = async (id) => {
    if (
      !window.confirm("Are you sure you want to delete this stock-out entry?")
    )
      return;
    ...
    <div className="total-stock-page">
            ...
            {stockSummary.length === 0 ? (
              ...
              stockSummary.map((item) => (
                <tr key={item._id}>
                  ...
                  <td
                    className={
                      Number(item.totalRemainingAmount) > 0 ? "pending" : "paid"
                    }
                  >
            ...
            {!loading && stocks.length === 0 ? (
              ...
              stocks.map((stock) => (
                <tr key={stock._id}>
                  ...
                  <td
                    className={
                      Number(stock.remainingAmount) > 0 ? "pending" : "paid"
                    }
                  >
      ...
      <div className="pagination-buttons">
        <button
          ...
          onClick={() => handlePageChange("stocks", "previous")}
        >
          Previous
        </button>
        <span className="page-info">
          Current Page: <strong>{filters.page}</strong> | Total Pages:{" "}
          <strong>{stockTotalPages}</strong> | Total Records:{" "}
          <strong>{stockTotalRecords}</strong>
        </span>
        <div>
          <button
            disabled={filters.page >= stockTotalPages}
            onClick={() => handlePageChange("stocks", "next")}
          >
            Next
          </button>
        </div>
      ...
      <div className="filters">
        ...
        />
        <select
          value={stockOutFilters.paymentStatus}
          onChange={(e) =>
            setStockOutFilters({
              ...stockOutFilters,
      ...
      <div className="pagination-buttons">
        <button
          disabled={stockOutFilters.page <= 1}
          onClick={() => handlePageChange("stockOut", "previous")}
        >
          Previous
        </button>
        <span className="page-info">
          Current Page: <strong>{stockOutFilters.page}</strong> | Total Pages:{" "}
          <strong>{stockOutTotalPages}</strong> | Total Records:{" "}
          <strong>{stockOutTotalRecords}</strong>
        </span>
        <div>
          <button
            disabled={stockOutFilters.page >= stockOutTotalPages}
            onClick={() => handlePageChange("stockOut", "next")}
          >
            Next
          </button>
        </div>
